...
public class MovieDao extends AbstractMFlixDao {
	...
	/**
	 * Returns all movies within the defined limit and skip values using a
	 * default descending sort key `tomatoes.viewer.numReviews`
	 *
	...
	/**
	 * Finds all movies that match the provide `genres`, sorted descending by
	 * the `sortKey` field.
	 *
	 ...
	 * aggregation pipeline { "$bucket": { "groupBy": "$runtime", "boundaries":
	 * [0, 60, 90, 120, 180], "default": "other", "output": { "count": {"$sum":
	 * 1} } } }
	 */
	...
	/**
	 * This method is the java implementation of the following mongo shell
	 * aggregation pipeline pipeline.aggregate([ ..., {$facet:{ runtime:
	 * {$bucket: ...}, rating: {$bucket: ...}, movies: {$addFields: ...}, }} ])
	 *
